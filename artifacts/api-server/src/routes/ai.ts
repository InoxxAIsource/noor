...
router.post("/ai/companion", requireAuth, async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    const response = await anthropic.messages.create({
      model: MODEL,
      max_tokens: 512,
      system: `You are Noor, a compassionate Islamic AI companion. Help Muslims with duas, Quran, fiqh, and daily spiritual practice with warmth and Islamic adab. Keep responses to 3-4 sentences. Do not reveal you are Claude.`,
      messages,
    });
...
router.post("/ai/director", requireAuth, async (req: AuthRequest, res: Response) => {
  ...
  if (usage >= AI_DAILY_LIMIT) {
    res.status(429).json({
      error: "You have reached your daily limit. JazakAllah khair for using Noor — come back tomorrow!",
      remaining: 0,
    });
    return;
  }
  ...
  try {
    ...
    const systemPrompt = `You are a warm, knowledgeable Islamic companion for Noor — "Remember Allah. Every day."
...
router.post("/ai/recommend", requireAuth, async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    const response = await anthropic.messages.create({
      model: MODEL,
      max_tokens: 400,
      system: "You are Noor app's dua recommendation engine. Return ONLY valid JSON. No markdown. No explanation.",
      messages: [
        {
          role: "user",
