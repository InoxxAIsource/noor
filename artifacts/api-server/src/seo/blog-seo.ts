...
export const BLOG_STUBS: Omit<BlogPost, "content_html">[] = [
  ...
  { slug: "noor-app-guide", title: "Noor App — Complete Guide to All Features", description: "Complete guide to all features of Noor Islamic Prayer App — prayer times, Quran, AI guide, names, and more.", category: "App", datePublished: "2026-01-01", internalLinks: [{ href: "/download", label: "Download Noor" }, { href: "/best-muslim-prayer-app-india", label: "Compare Apps" }, { href: "/prayer-times", label: "Prayer Times" }] },
...
router.get("/blog", async (_req: Request, res: Response) => {
  ...
  const head = seoHead({
    ...
    description: "Islamic articles about prayer times, Quran, duas, Muslim names, Ramadan, Zakat, and spiritual growth. Learn and grow with Noor.",
    canonical: "/blog",
...
router.get("/blog/:slug", async (req: Request, res: Response) => {
  ...
  const content_html = dbPost?.content_html || `<p style="color:#4a7a4a;padding:20px;background:#002800;border-radius:8px">Full article content is being generated by our AI. Check back soon or <a href="/download" style="color:#00a550">download Noor app</a> for more Islamic content.</p>`;
  ...
  const blogSchema = {
    ...
    "wordCount": wordCount,
    "author": { "@type": "Organization", "name": "Noor App" },
    "publisher": { "@type": "Organization", "name": "Noor App" },
    "url": `https://noorapp.com/blog/${slug}`,
  };
